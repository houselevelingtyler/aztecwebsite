function scrollsequenceMainFunction() {
    if (document.getElementsByClassName('scrollsequence-wrap')[0]) {
        ...
        function ssqCurCalculate(scroll_pos) {
            ...
            let t1 = performance.now();
            let tdelta = (t1 - t0).toFixed(2);
            (DEBUG && !SHOWMARKERS) && console.log(tdelta, 'ms ssqCur(', scroll_pos, '):', ssqCur);
        ...
        function ssqDumbPreloadArray(curImg, imageTotalLength) {
            let t0 = performance.now(); //cur img is absolute
            ...
            let t1 = performance.now();
            let tdelta = (t1 - t0).toFixed(2);
            PRELOADDEBUG && console.log(tdelta, 'ms ssqImg.dumbPreloadOrder:', ssqImg.dumbPreloadOrder)
        ...
        function preloadPriorityAndNonPriorityImages() {
            ...
            let t1 = performance.now();
            let tdelta = (t1 - t0).toFixed(2);
            PRELOADDEBUG && console.log(tdelta, 'ms preloadPriorityAndNonPriorityImages', 'PRIORITY:', ssqImg.priorityImageArray, 'NON-PRIORITY:', ssqImg.nonPriorityImageArray, 'NON PRIORITY REQD: ', ssqImg.nonPriorityImageCountRequired)
        ...
        function imageWasLoaded(imageInfo, isPriority) {
            ...
            if (ssqImg.priorityImagesPreloadedCount >= ssqImg.priorityImageArray.length) {
                PRELOADDEBUG && console.log('All priority images have been loaded. Loading non-priority now. ssqImg.nonPriorityImagesPreloadedCount:', ssqImg.nonPriorityImagesPreloadedCount, '/', ssqImg.nonPriorityImageCountRequired)
        ...
        function renderCanvas(sc, page, image, requestor, info) { // Check screen size, update canvas size (css + canvas) and render with scale and align
            ...
            if (ssqImg.objArray[sc][page][image].aaaLoaded) {
                ...
                PRELOADDEBUG && console.log('Image [', sc, ',', page, ',', image, '] was not loaded in time ')
            }; // updated 1.3.8 so it only shows when preloaddebug
        }
        /**
        ...
        if (SHOWMARKERS) {
            ssqHtml[0].ssqalert.style.display = "block";
        ...
        function ssqTimelines7() {
            ...
            for (let s = 0; s < ssqInput.scLength; s++) {
                for (let i = 0; i < ssqInput.sc[s].pageLength; i++) {
                    if (!ssqCur.image) {
                        ssqCur.image = 0;
                    }; // since 1.0.078 - to prevent gsap error. This line was added to prevent GSAP warning message https://wordpress.org/support/topic/gsap-invalid-property-error/#post-14328432
                    timelineSc[s].set(ssqHtml[s].page[i], {
                        display: "block"
                    }, ssqInput.sc[s].page[i].pageAbsBegin) // Turn pages ON 
                    ...
                    timelineSc[s].set(ssqHtml[s].page[i], {
                        display: "none"
                    }, ssqInput.sc[s].page[i].pageAbsEnd) // Turn pages OFF 
                } // end of i loop
            } // end of s loop
            // STEP 3 - TWEEN AnimEl 
            for (let s = 0; s < ssqInput.scLength; s++) {
        ...
        function ssqFirstDraw() {
            ...
            function myDelFunction(param1, param2) {
                gsap.to('.scrollsequence-wrap', {
                    autoAlpha: 1,
                    duration: 1.25
                }); // With this are two lines associated in -admin.php (script and noscript) // removed 1.0.074
                ScrollTrigger.refresh()
                // ACTIVE SC: 
                if (ssqInit.active.isActive) {
